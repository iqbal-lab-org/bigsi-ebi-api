...
def search_bigsi_and_update_results(url, sequence_search):
    logger.info(search_bigsi_and_update_results.__name__)
    logger.debug('sequence_search_id: %s', sequence_search.id)
...
def variant_search_bigsi_and_update_results(url, variant_search):
    bigsi_client = BigsiClient(url)
